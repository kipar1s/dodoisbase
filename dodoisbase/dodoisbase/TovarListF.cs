...
namespace dodoisbase
{
    public partial class TovarListF : Form
    {
        ...
        {
            // Создаем новый товар. 
            // Благодаря конструктору Tovar() { Ингредиент = new Ingr(); }, который мы обсуждали, 
            // здесь не будет NullReferenceException.
            Tovar new_tovar = new Tovar();

            TovarFormU tovar_form_unit = new TovarFormU();
            tovar_form_unit.SetDataSourse(new_tovar);

            if (tovar_form_unit.ShowDialog() == DialogResult.OK)
            {
        ...
        private void tsb_Edit_Click(object sender, EventArgs e)
        {
            ...
            TovarFormU tovar_form_unit = new TovarFormU();
            tovar_form_unit.SetDataSourse(selected_tovar);
